...
leadSNPs = read.delim("plots/tracks/sentinels_hg19.bed", header= F)%>% dplyr::rename(chr = V1, start = V2, end = V3, rsid = V4)
sumstats = fread("CTS2022_summarystats/") 
...
regions_of_interest = lapply(leadSNPs$rsid,  function(leadSNP_rsid){
	leadSNP_panel = sumstats[sumstats$rsID == leadSNP_rsid,]
	leadSNP_panel_of_interest = sumstats[sumstats$Chr == leadSNP_panel$chromosome & sumstats$position > leadSNP_panel$position-500000 & sumstats$position < leadSNP_panel$position+500000,]
